...
% For inclusion in the PRT this functio has been modified slightly. Global
% variables have been eliminated and minor changes have been made to reduce
% mLint messages. Most notably though, a subfunction safefzero has been
% added that returns zero as t_star when fzero errors. This happens when
% the optimal bandwidth is zero which happens for some small datasets with
% repeated values.
...
t_star = safefzero( @(t)(t-evolve(t,N,I,A2)),[0,0.1]);
...
for i=1:ncols
    [dum,bins(:,i)] = histc(data(:,i),[0:1/M:1],1); %#ok<ASGLU>
    bins(:,i) = min(bins(:,i),M);
end
...
function out = safefzero(fun,lims)

try
    out = fzero(fun,lims);
catch %#ok<CTCH>
    % Sometimes (with small datasets with repeated values, the best
    % bandwidth is 0. This doesn't play nice with the fzero and the
    % assumptions made with this method/code.
    out = 0;
end
