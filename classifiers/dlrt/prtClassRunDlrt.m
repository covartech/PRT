...
h0 = getObservations(PrtDlrt.PrtDataSet,find(getLabels(PrtDlrt.PrtDataSet)==0));
h1 = getObservations(PrtDlrt.PrtDataSet,find(getLabels(PrtDlrt.PrtDataSet)==1));
...
for i = 1:memSize:DataSet.nObservations
    if ~PrtOptions.ignoreH0
        ...
        y(indices) = log(n0./n1) + DataSet.nDimensions*log(dH0./dH1);
    else
        indices = i:min([i+memSize-1,DataSet.nObservations]);
        dH1 = sort(obj.distanceFn(getObservations(DataSet,indices),h1),2,'ascend');
        dH1 = dH1(:,obj.k);
        
        y(indices) = -log(n1) - DataSet.nDimensions*log(dH1);
    end
end
