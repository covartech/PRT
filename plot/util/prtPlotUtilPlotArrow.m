...
% 
% if nargin < 3 || isempty(arrowSize)
%     arrowSize = (x(end)-x(1))*0.01;
% end
% 
% holdState = get(gca,'NextPlot');
% hLine = plot(x,y,'k','linewidth',1.5);
% 
...
% % Totally wrong....
% 
% %x(end)^2 -2*x(end)*newX + newX^2 + (y(end)+intercept)^2 - 2*newX*slope*(y(end)+intercept) + (newX*slope)^2 - arrowSize^2
% %newX^2*(1 + slope^2) + newX * (-2*x(end) - 2*slope*(y(end)+intercept)) + x(end)^2+(y(end)+intercept)^2 - arrowSize^2
% 
% intersectionPointX1 = (x(end) + intercept*slope + slope*y(end) - (arrowSize^2*slope^2 + arrowSize^2 - intercept^2 + 2*intercept*slope*x(end) - 2*intercept*y(end) - slope^2*x(end)^2 + 2*slope*x(end)*y(end) - y(end)^2)^(1/2))/(slope^2 + 1);
% intersectionPointX2 = (x(end) + intercept*slope + slope*y(end) + (arrowSize^2*slope^2 + arrowSize^2 - intercept^2 + 2*intercept*slope*x(end) - 2*intercept*y(end) - slope^2*x(end)^2 + 2*slope*x(end)*y(end) - y(end)^2)^(1/2))/(slope^2 + 1);
% 
% 
% %intersectionPoint = cat(2,x(2)-arrowSize,y(2)-arrowSize*slope);
% 
% newIntercept = intersectionPoint(2) + intersectionPoint(1)/slope;
% newSlope = -1/slope;
% 
% 
% solutionX1 = (2*intersectionPoint(1) + (arrowSize^2*newSlope^2 + arrowSize^2 - 4*intersectionPoint(1)^2*newSlope^2 + 8*intersectionPoint(1)*intersectionPoint(2)*newSlope - 8*intersectionPoint(1)*newSlope*newIntercept - 4*intersectionPoint(2)^2 + 8*intersectionPoint(2)*newIntercept - 4*newIntercept^2)^(1/2) + 2*intersectionPoint(2)*newSlope - 2*newSlope*newIntercept)/(2*newSlope^2 + 2);
% solutionX2 = (2*intersectionPoint(1) - (arrowSize^2*newSlope^2 + arrowSize^2 - 4*intersectionPoint(1)^2*newSlope^2 + 8*intersectionPoint(1)*intersectionPoint(2)*newSlope - 8*intersectionPoint(1)*newSlope*newIntercept - 4*intersectionPoint(2)^2 + 8*intersectionPoint(2)*newIntercept - 4*newIntercept^2)^(1/2) + 2*intersectionPoint(2)*newSlope - 2*newSlope*newIntercept)/(2*newSlope^2 + 2);
%  
% solutionY1 = solutionX1*newSlope + newIntercept;
% solutionY2 = solutionX2*newSlope + newIntercept;
% 
% hHead = patch(cat(1,solutionX1,solutionX2, x(end)),cat(1,solutionY1,solutionY2, y(end)),[0 0 0]);
% 
% set(gca,'NextPlot',holdState);
% 
% varargout = {};
% if nargout
%     varargout = {hLine, hHead};
% end